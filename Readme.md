...
- Производительность
  ...
  Замеры производительности сортировки и математических вычислений на разных версиях .Net.
...
1. Создание и конфигурирование NamedPipe: Сервер создает и конфигурирует
   именованный трубопровод с определенным именем, которое будет доступно клиенту. Клиенту необходимо
   знать это имя, чтобы подключиться к трубе.
2. Ожидание подключения: Сервер начинает ожидать подключения клиента к именованной трубе.
   NamedPipe. Это блокирующая операция, и сервер остается в подвешенном состоянии до тех пор, пока клиент не подключится.
   пока клиент не подключится.
3. Подключение к именованной трубе: Клиент инициирует подключение к именованному трубопроводу, указывая имя трубы, к которой он хочет подключиться.
4. Обмен данными: После успешного соединения клиент и сервер могут обмениваться
   данными в виде байтовых потоков. Клиент отправляет запросы на выполнение бизнес-логики,
   а сервер обрабатывает эти запросы и отправляет их в виде байтовых потоков.
   логики, а сервер обрабатывает эти запросы и отсылает результаты.
...
Вы можете изменить это поведение или использовать любое имя в рамках своего проекта.
...
`PipeDirection` указывает направления канала, `PipeDirection.In` говорит о том что сервер будет только принимать сообщения.
...
Например, в .Net framework 4.8 значение `PipeOptions.CurrentUserOnly` нет, но появилось в .Net 7.
...
Пример кода полностью доступен в репозитории, вы можете продебажить его выполнив несколько шагов:
...
Производные классы вы можете найти в репозитории проекта: https://github.com/Nice3point/InterprocessCommunication/TwoWay/Backend/Server/PipeProtocol.cs

Для того чтобы сервер мог отправлять ответы клиенту, мы должны модифицировать класс ServerDispatcher.
Это позволит записывать ответы в поток после выполнения задачи.

А так же изменим направление трубы на двунаправленное.
...
В настоящее время программа Revit использует .Net Framework 4.8. 
Однако для улучшения пользовательского интерфейса (UI) нашего плагина мы рассматриваем переход на .Net 7. 
Важно отметить, что бэкэнд нашего плагина будет взаимодействовать только с Revit на устаревшем Framework, и будет выступать в качестве сервера.
